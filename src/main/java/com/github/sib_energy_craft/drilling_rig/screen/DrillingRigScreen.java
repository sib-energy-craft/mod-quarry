...
public class DrillingRigScreen extends HandledScreen<DrillingRigScreenHandler> {
    ...
    protected void drawBackground(@NotNull DrawContext drawContext, float delta, int mouseX, int mouseY) {
        ...
        drawContext.drawTexture(TEXTURE, x + 84, y + 36, 176, 0, 7, progress);

        var startPosition = this.handler.getStartPosition();
        if(startPosition.x > -AbstractDrillingRigBlockEntity.MAX_OFFSET) {
            drawContext.drawTexture(TEXTURE, x + MOVE_LEFT_X, y + 46, 183, 0, 12, 12);
            if(isMouseOver(mouseX, mouseY, x + MOVE_LEFT_X, y + 46, 12, 12)) {
                drawContext.drawTexture(TEXTURE, x + MOVE_LEFT_X, y + 46, 183, 12, 12, 12);
            }
        }
        if(startPosition.x < AbstractDrillingRigBlockEntity.MAX_OFFSET) {
            drawContext.drawTexture(TEXTURE, x + MOVE_RIGHT_X, y + 46, 207, 0, 12, 12);
            if(isMouseOver(mouseX, mouseY, x + MOVE_RIGHT_X, y + 46, 12, 12)) {
                drawContext.drawTexture(TEXTURE, x + MOVE_RIGHT_X, y + 46, 207, 12, 12, 12);
            }
        }
        if(startPosition.y < AbstractDrillingRigBlockEntity.MAX_OFFSET) {
            drawContext.drawTexture(TEXTURE, x + MOVE_UP_X, y + 34, 195, 0, 12, 12);
            if(isMouseOver(mouseX, mouseY, x + MOVE_UP_X, y + 34, 12, 12)) {
                drawContext.drawTexture(TEXTURE, x + MOVE_UP_X, y + 34, 195, 12, 12, 12);
            }
        }
        if(startPosition.y > -AbstractDrillingRigBlockEntity.MAX_OFFSET) {
            drawContext.drawTexture(TEXTURE, x + MOVE_DOWN_X, y + 58, 219, 0, 12, 12);
            if(isMouseOver(mouseX, mouseY, x + MOVE_DOWN_X, y + 58, 12, 12)) {
                drawContext.drawTexture(TEXTURE, x + MOVE_DOWN_X, y + 58, 219, 12, 12, 12);
            }
        }

        var size = this.handler.getSize();
        if(size.x > 1) {
            drawContext.drawTexture(TEXTURE, x + WIDTH_UP_X, y + 34, 231, 0, 11, 12);
            if(isMouseOver(mouseX, mouseY, x + WIDTH_UP_X, y + 34, 11, 12)) {
                drawContext.drawTexture(TEXTURE, x + WIDTH_UP_X, y + 34, 231, 12, 11, 12);
            }
        }
        if(size.x < AbstractDrillingRigBlockEntity.MAX_SIZE) {
            drawContext.drawTexture(TEXTURE, x + WIDTH_DOWN_X, y + 34, 242, 0, 11, 12);
            if(isMouseOver(mouseX, mouseY, x + WIDTH_DOWN_X, y + 34, 11, 12)) {
                drawContext.drawTexture(TEXTURE, x + WIDTH_DOWN_X, y + 34, 242, 12, 11, 12);
            }
        }

        if(size.y > 1) {
            drawContext.drawTexture(TEXTURE, x + HEIGHT_UP_X, y + 58, 231, 0, 11, 12);
            if(isMouseOver(mouseX, mouseY, x + HEIGHT_UP_X, y + 58, 11, 12)) {
                drawContext.drawTexture(TEXTURE, x + HEIGHT_UP_X, y + 58, 231, 12, 11, 12);
            }
        }
        if(size.y < AbstractDrillingRigBlockEntity.MAX_SIZE) {
            drawContext.drawTexture(TEXTURE, x + HEIGHT_DOWN_X, y + 58, 242, 0, 11, 12);
            if(isMouseOver(mouseX, mouseY, x + HEIGHT_DOWN_X, y + 58, 11, 12)) {
                drawContext.drawTexture(TEXTURE, x + HEIGHT_DOWN_X, y + 58, 242, 12, 11, 12);
            }
        }
        ...
        drawContext.drawTextWithShadow(textRenderer, offsetText, x + offsetTextX, y + 18, whiteRgb);
        ...
        drawContext.drawTextWithShadow(textRenderer, widthText, x + widthTextX, y + 36, whiteRgb);
        ...
        drawContext.drawTextWithShadow(textRenderer, heightText, x + heightTextX, y + 60, whiteRgb);
